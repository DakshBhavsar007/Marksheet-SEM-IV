...
PDF_PATH = r'c:\Users\parul\Desktop\marksheet\marksheet_SEM-IV\Compile _Marksheet_SEM_IV_CE_IT_2026_T3_FSD-2.pdf'
...
pdf_records = {}   # enrollment -> T3 FSD-2 mark

with pdfplumber.open(PDF_PATH) as pdf:
    total_pages = len(pdf.pages)
    print(f"Total pages to parse: {total_pages}")
    for page_num, page in enumerate(pdf.pages):
        print(f"Parsing page {page_num + 1}/{total_pages}...")
...
print(f"Successfully parsed {len(pdf_records)} records from the T3 FSD-2 PDF.")
...
for student in data:
    ...
    if enroll in pdf_records:
        ...
        student["fsd23"] = t3_mark
        
        # Add T3 mark to cumulative FSD2 column
        existing_fsd2 = student.get("fsd2", 0.0) or 0.0
        student["fsd2"] = round(float(existing_fsd2) + t3_mark, 1)
        ...
        # For students not found, we still make sure their total is recalculated/updated if needed
        dm = float(student.get("dm", 0.0) or 0.0)
        coa = float(student.get("coa", 0.0) or 0.0)
        toc = float(student.get("toc", 0.0) or 0.0)
        fsd2 = float(student.get("fsd2", 0.0) or 0.0)
