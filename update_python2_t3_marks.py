...
PDF_PATH = r'c:\Users\parul\Desktop\marksheet\marksheet_SEM-IV\Compile_Marksheet_SEM_IV_CE_IT_2026_T3_FCSP-2.pdf'
...
pdf_records = {}   # enrollment -> T3 Python-2 mark
...
print(f"Successfully parsed {len(pdf_records)} records from the T3 Python-2 (FCSP-2) PDF.")
...
for student in data:
    ...
    if enroll in pdf_records:
        ...
        student["python23"] = t3_mark

        # Add T3 mark to cumulative Python2 column
        existing_python2 = student.get("python2", 0.0) or 0.0
        student["python2"] = round(float(existing_python2) + t3_mark, 1)
        ...
        # For students not found, recalculate total to keep consistent
        dm   = float(student.get("dm",   0.0) or 0.0)
        coa  = float(student.get("coa",  0.0) or 0.0)
        toc  = float(student.get("toc",  0.0) or 0.0)
        fsd2 = float(student.get("fsd2", 0.0) or 0.0)
