...
PDF_PATH = r'c:\Users\parul\Desktop\marksheet\marksheet_SEM-IV\FSD_2_COMPILE_MARKSHEET_SEM_IV_CE_IT_2026_T2.pdf'
...
pdf_records = {}   # enrollment -> T2 FSD2 mark
...
print(f"Parsed {len(pdf_records)} student records from T2 FSD2 PDF")
...
# ── ADD T2 FSD2 marks to existing fsd2 field ───────────────────────────────
updated  = 0
not_found = []
...
for student in data:
    ...
    if enrollment in pdf_records:
        ...
        existing_fsd2 = student.get('fsd2', 0) or 0
        
        student['fsd2'] = round(float(existing_fsd2) + t2, 1)
        student['fsd22'] = t2
        
        # Re-calculate total
        total = float(student.get('dm', 0)) + float(student.get('coa', 0)) + float(student.get('toc', 0)) + float(student.get('fcsp', 0)) + student['fsd2']
...
print(f"Updated {updated} students (fsd2 = T1 + T2 combined, fsd22 added)")
print(f"{len(not_found)} students NOT found in T2 FSD2 PDF (unchanged)")
if not_found:
