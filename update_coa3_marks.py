...
PDF_PATH = r'c:\Users\parul\Desktop\marksheet\marksheet_SEM-IV\Compile_Marksheet_SEM_IV_CE_IT_2026_T3_COA.pdf'
...
pdf_records = {}   # enrollment -> T3 COA mark
...
print(f"Successfully parsed {len(pdf_records)} records from the T3 COA PDF.")
...
for student in data:
    ...
    if enroll in pdf_records:
        ...
        student["coa3"] = t3_mark
        
        # Add T3 mark to cumulative COA column
        existing_coa = student.get("coa", 0.0) or 0.0
        student["coa"] = round(float(existing_coa) + t3_mark, 1)
        ...
        # For students not found, we still make sure their total is calculated/updated if needed
        # (Though we aren't changing their COA T3, keeping consistency is good)
        dm = float(student.get("dm", 0.0) or 0.0)
        coa = float(student.get("coa", 0.0) or 0.0)
        toc = float(student.get("toc", 0.0) or 0.0)
        fsd2 = float(student.get("fsd2", 0.0) or 0.0)
